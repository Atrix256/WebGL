...
const char *CShaderQuartic3d::GetVertexShader()
{
    return
    SHADER_SOURCE(
...
const char *CShaderQuartic3d::GetFragmentShader()
{
    return
    SHADER_SOURCE(
    ...
    vec4 SampleTime(vec3 time, bool linearSampling) {
        ...
        if (linearSampling)
            return texture(uSampler, (time + 0.5) / vec3(2.0, 4.0, 2.0));
        ...
        vec4 A = texture(uSampler, (floorTime + vec3(0.0, 0.0, 0.0)) / vec3(2.0, 4.0, 2.0));
        vec4 B = texture(uSampler, (floorTime + vec3(1.0, 0.0, 0.0)) / vec3(2.0, 4.0, 2.0));
        vec4 C = texture(uSampler, (floorTime + vec3(0.0, 1.0, 0.0)) / vec3(2.0, 4.0, 2.0));
        vec4 D = texture(uSampler, (floorTime + vec3(1.0, 1.0, 0.0)) / vec3(2.0, 4.0, 2.0));
        vec4 front = mix(mix(A, B, frac.x), mix(C, D, frac.x), frac.y);

        vec4 E = texture(uSampler, (floorTime + vec3(0.0, 0.0, 1.0)) / vec3(2.0, 4.0, 2.0));
        vec4 F = texture(uSampler, (floorTime + vec3(1.0, 0.0, 1.0)) / vec3(2.0, 4.0, 2.0));
        vec4 G = texture(uSampler, (floorTime + vec3(0.0, 1.0, 1.0)) / vec3(2.0, 4.0, 2.0));
        vec4 H = texture(uSampler, (floorTime + vec3(1.0, 1.0, 1.0)) / vec3(2.0, 4.0, 2.0));
