...
The De Casteljeau algorithm is equivelant to the Bernstein form of Bezier curves (\autoref{eqn:bernsteinform}) which means that the Bernstein form must be able to be eqvaluated by linear, bilinear, trilinear, and N-Linear interpolation.

\begin{equation}
\bf{P} = \sum\limits_{i=0}^n\binom {n} {i}s^{n-i}t^i\text{\bf{P}}_i
\label{eqn:bernsteinform}
\end{equation}

When n is 1, to make a linear Bezier curve, the equation we get out is in fact just linear interpolation (\autoref{eqn:bernsteinformn1}).  Note that in the Berntstein form, s is equivelant to 1-t.
...
When n is 2, to make a quadratic Bezier curve, things are not quite as obvious:



TODO: finish this section.
TODO: show linear, bilinear, trilinear interpolation to be equivelant!
TODO: How to prove for N dimensions?
