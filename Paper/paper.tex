...
A limitation with this technique is that control points can only take values that can be stored and recalled from the specific texture format you are using.  This can be an issue not only for the range of values possibly - such as when a texture can only hold values from 0 to 1 or 0 to 255 - but is also an issue for accuracy due being interpolated in a 9 bit (1.8) fixed point format \cite{NVIDIA}.   The value range problem is easily solved by scaling and offsetting.  The accuracy problem is not as easily addressed, but there are some ways mentioned later in the paper.

The accuracy issues can be seen in \autoref{fig:quickaccuracy}, although the accuracy is still significantly higher than a baked out curve using the same number of pixels as seen in \autoref{fig:quickcomparison}.

TODO: the 9 bit fixed point format doesn't belong mentioned where it is.
TODO: also need to find and mention the thing about how interpolation weights are multiplied before taking it into floating point.
